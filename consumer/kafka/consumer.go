...
func NewConsumer(subscriberName string, event retriable.Event, brokers []string, options ...Option) *kConsumer {
	...
	// make publisher
	c.publisher = kafka.NewProducer(event, c.conf.Brokers)
...
func (k *kConsumer) Consume(ctx context.Context, handlerFunc consumer.HandleFunc) error {
	var err error
...
func (k *kConsumer) BatchConsume(ctx context.Context, handlerFunc consumer.BatchHandleFunc) error {
	var err error
	k.consumerGroup, err = sarama.NewConsumerGroup(k.conf.Brokers, k.subscriberName, k.conf.KafkaCfg)
	if err != nil {
		return err
